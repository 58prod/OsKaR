...
/*
 * Diagnostic classique : l'ancienne version (ressenti + 3 critères par pilier),
 * gardée telle quelle depuis que /diagnostic est passé à la version à 20
 * pratiques (2026-09-24). Hors menu, non indexée ; elle rouvre les bilans
 * enregistrés avec l'ancienne version (« Mes bilans », restauration par email).
 */

/** Mode de la modale de saisie d'email (invité). */
type EmailPromptMode = 'save' | 'pdf' | 'restore';

const DiagnosticClassiquePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    DiagnosticsService.getById(bilanDemande)
      .then((record) => {
        // Un bilan de la version actuelle s'ouvre dans /diagnostic.
        if (record?.type === 'organisation' && record.version === 4) {
          router.replace(`/diagnostic?bilan=${record.id}`);
          return;
        }
        if (record?.type === 'organisation') {
          setState(record.responses as DiagnosticState);
  ...
  const handleEmailSubmit = useCallback(async (email: string, accepteRecontact = false) => {
    ...
    if (emailPromptMode === 'restore') {
      ...
      try {
        ...
        if (record.version === 4) {
          toast.info('Votre dernier bilan a été fait avec la version actuelle du diagnostic : il s’ouvre dans /diagnostic.');
          router.push(`/diagnostic?bilan=${record.id}`);
          return;
        }
        setState(record.responses as DiagnosticState);
        setCurrentEmail(email);
  ...
  }, [state, user, emailPromptMode, createDiagnostic, sendPdf, toast, router]);
  ...
  return (
    ...
    >
      ...
      <div role="note" className="mb-6 rounded-lg border border-dashed border-navy/30 bg-white px-4 py-3 text-[13px] text-muted">
        <strong className="text-navy">Ancienne version du diagnostic.</strong> Elle reste disponible pour rouvrir les bilans qui ont été faits avec elle.{' '}
        <Link href="/diagnostic" className="font-semibold text-navy underline">Faire le diagnostic actuel</Link>
      </div>
...
export default DiagnosticClassiquePage;
