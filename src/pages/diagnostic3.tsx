...
import { PILIERS3 } from '@/lib/diagnostic3/contenu';
import { analyser3, etatInitial3, nbPreuves, niveau3, note3, piliersAttendus3, type Etat3 } from '@/lib/diagnostic3/calcul';

/*
 * Diagnostic 3 — version d'essai rapide, à comparer avec /diagnostic et
 * /diagnostic2. Même format que le Diagnostic en ligne (ressenti + cases),
 * critères vérifiables, nouveau calcul (lib/diagnostic3/calcul.ts) et
 * analyse réécrite. Hors menu, non indexée, rien n'est enregistré.
 */

const Radar3 = dynamic(() => import('@/components/diagnostic3/Radar3'), { ssr: false });
...
const Diagnostic3Page: React.FC = () => {
  const [etat, setEtat] = useState<Etat3>(etatInitial3);
  const [revele, setRevele] = useState(false);
  const isAuthenticated = useAppStore((s) => s.isAuthenticated);
  const authReady = useAppStore((s) => s.authReady);
  const analyseRef = useRef<HTMLElement>(null);

  const analyse = useMemo(() => analyser3(etat), [etat]);
  const attendus = piliersAttendus3(etat).length;
  ...
  const cocher = useCallback((id: PillarId, i: number) => {
    setEtat((prev) => {
      const preuves = [...prev.piliers[id].preuves] as Etat3['piliers'][PillarId]['preuves'];
      preuves[i] = !preuves[i];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-6 rounded-lg border border-dashed border-navy/30 bg-white px-4 py-3 text-[13px] text-muted">
        ...
        <p>
          <strong className="text-navy">Version d’essai 3</strong> — format rapide à cases. À comparer avec le{' '}
          <Link href="/diagnostic" className="font-semibold text-navy underline">Diagnostic actuel</Link> et la{' '}
          <Link href="/diagnostic2" className="font-semibold text-navy underline">version 2</Link>. Rien n’est enregistré.
        </p>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_340px] items-start">
        <div>
          {PILLARS.map((pilier, index) => {
            ...
            const contenu = PILIERS3[pilier.id];
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                  ...
                  {note !== null && !ecarte && (
                    <div className="flex items-center gap-3 sm:block sm:text-right shrink-0">
                      ...
                      <div className="sm:mt-1.5"><Pastille niveau={niveau3(note)} /></div>
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-col gap-4 lg:sticky lg:top-24">
          ...
          <section className="bg-white rounded-card border border-line shadow-card p-5" aria-label="Radar ressenti et preuves">
            <div className="text-sm font-bold text-navy mb-1">Ressenti ou réalité&nbsp;?</div>
            <div className="flex items-center gap-4 text-[11.5px] text-muted mb-2">
              ...
              <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-teal/30 border border-navy" aria-hidden /> Vos preuves</span>
            </div>
            <div className="w-full h-56"><Radar3 valeurs={valeursRadar} /></div>
          </section>
        ...
        <section ref={analyseRef} className="mt-8 scroll-mt-24 space-y-5 animate-fade-in" aria-label="Votre analyse">
          ...
          {analyse.priorite && (
            <div className="rounded-card bg-white border border-line shadow-card p-6 border-l-4" style={{ borderLeftColor: stateBorder(analyse.priorite.niveau) }}>
              ...
              {analyse.domino && analyse.domino.texte && (
                <p className="mt-4 rounded-lg bg-surface px-4 py-3 text-sm text-ink leading-relaxed">
                  <strong className="text-navy">Effet domino sur {analyse.domino.fort.label}, votre point fort · </strong>
                  {analyse.domino.texte}
                </p>
              )}
            </div>
          )}
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="rounded-card bg-white border border-line shadow-card p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider text-muted">Votre ressenti face aux preuves</span>
              </div>
              {analyse.lucidite.length === 0 ? (
                <p className="text-[15px] text-ink leading-relaxed">
                  <strong className="text-navy">Votre regard est lucide.</strong> Sur chaque pilier, votre ressenti rejoint vos preuves.
                  C’est un vrai atout : vous décidez sur une image juste de votre entreprise.
                </p>
              ) : (
                <ul className="space-y-3">
                  {analyse.lucidite.map((l) => (
                    <li key={l.id} className="text-sm text-ink leading-relaxed">
                      <strong className="text-navy">{l.sens === 'angle-mort' ? `Angle mort · ${l.label}` : `Force sous-estimée · ${l.label}`}</strong>
                      <br />
                      Vous vous donnez {l.ressenti}/10, vos preuves disent {fmt(l.preuves)}.{' '}
                      <span className="text-muted">
                        {l.sens === 'angle-mort'
                          ? 'C’est souvent là que se cachent les mauvaises surprises.'
                          : 'Vous êtes plus avancé que vous ne le pensez : faites-le savoir à votre équipe.'}
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="rounded-card bg-white border border-line shadow-card p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider text-muted">Vos actions pour les 30 prochains jours</span>
              </div>
              {analyse.leviers.length === 0 ? (
                <p className="text-[15px] text-ink leading-relaxed">Toutes vos cases sont cochées : l’enjeu est de le rester. Refaites le diagnostic dans trois mois.</p>
              ) : (
                <ol className="space-y-3">
                  {analyse.leviers.map((l, i) => (
                    <li key={i} className="flex gap-3">
                      ...
                      <span className="text-sm text-ink leading-relaxed"><strong className="text-navy">{l.label} · </strong>{l.action}</span>
                    </li>
                  ))}
                </ol>
              )}
          ...
          <div className="rounded-card bg-navy-dark p-6 sm:p-8 text-white">
            ...
            {analyse.priorite ? (
              <>
                <p className="text-2xl font-extrabold leading-snug max-w-2xl">Passez du constat à l’action sur {analyse.priorite.label}.</p>
              ...
              <>
                <p className="text-2xl font-extrabold leading-snug max-w-2xl">Gardez ce niveau en grandissant.</p>
                <p className="text-[15px] text-white/75 mt-2 max-w-2xl leading-relaxed">Suivez vos objectifs et animez vos rituels d’équipe avec Oskar, pour que ce qui marche aujourd’hui tienne demain.</p>
              </>
            )}
            ...
            <p className="text-xs text-white/45 mt-4">Refaites ce diagnostic dans trois mois : vous verrez vos progrès sur le radar.</p>
          </div>
        </section>
      )}
    </AppShell>
...
export default Diagnostic3Page;
