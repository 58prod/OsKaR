...
import { IDEABOX_ACCENT, VOTE_LIMITS, type IdeaboxState } from './ideaboxLogic';
...
/**
 * Barre supérieure de la Boîte à idées, sur le modèle des autres jeux :
 * compteurs de séance à gauche ; réglages (anonymat, cœurs par personne),
 * export, réinitialisation confirmée et minuteur partagé à droite.
 */
export const IdeaboxToolbar: React.FC<IdeaboxToolbarProps> = ({
  state, remainingSec, publishedCount, retainedCount, votesCount, isFacilitator,
  ...
  return (
    <div
      ...
      style={{ '--tool-accent': IDEABOX_ACCENT } as React.CSSProperties}
    >
          ...
          <>
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-line px-3 py-1.5 text-sm font-semibold text-navy transition-colors hover:bg-surface">
              <input
                ...
                className="h-4 w-4"
                style={{ accentColor: IDEABOX_ACCENT }}
              />
            ...
            <label className="flex items-center gap-2 text-sm font-semibold text-muted">
              ...
              >
                {!VOTE_LIMITS.some((v) => v.value === state.voteLimit) && (
                  <option value={state.voteLimit}>{state.voteLimit} cœurs</option>
                )}
                {VOTE_LIMITS.map((v) => (
                  <option key={v.value} value={v.value}>{v.value ? `${v.label} par personne` : 'Cœurs illimités'}</option>
                ))}
              </select>
            </label>
            <button
        ...
        >
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-card-hover">
            ...
            <p className="mt-2 text-sm text-muted">
              Toutes les idées, votes et sélections de la séance seront effacés pour tout le monde. Pensez à exporter le résumé avant.
            </p>
